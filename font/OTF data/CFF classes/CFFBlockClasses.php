...
class FDSelectIndex {
	...
	function get_FD( $glyphindex ) {
		if ( $this->format == 0 ) {
			...
			if ( $glyphindex <= $this->sentinel ) {
				return $this->Range3[ $range3 - 1 ][ 'fd' ];
			} // if it's not, we have a problem, and a mystery glyph that should not exist!
			else {
				echo "ERROR IN FETCHING FONT DICT FOR GLYPH $glyphindex (none found!!)\n";
				return false;
			}
		}
...
class CFFIndex {
	...
	function data_as_strings() {
		$this->data_as_strings = true;
	}
	...
	function get_data( &$fh, $offset_index ) {
		...
		$data = '';
		while ( $rb-- > 0 ) {
			$data .= chr( FileRead::read_BYTE( $fh ) );
		}
		rewind( $fh );
		fseek( $fh, $mark );
		return $data;
	...
	// static create
	static function create_index( &$fh, $cff_offset = NULL, $charstringtype = NULL ) {
		$index = new CFFIndex();
		$index->setup( $fh );
		return $index;
	}
	...
	function toString() {
		...
		$ssess = array();
		for ( $o = 0; $o < count( $this->offsets ); $o++ ) {
			$ssess[ ] = "[$o]=>[" . $this->offsets[ $o ] . "]";
		}
		$ret .= implode( ",", $ssess ) . "}}";
		$ret .= "\n";
		return $ret;
...
class CFFTopDictIndex extends CFFIndex {
	...
	public function is_CIDFont() {
		return $this->ROS != array();
	}
...
class CFFPrivateDict {
	...
	function load_subroutines( &$fh, $local_offset, $charstringtype ) {
		if ( $this->Subrs != '-' ) {
			...
			if ( $charstringtype == 1 ) {
				$this->Subroutinesbias = 0;
			} elseif ( $nSubrs < 1240 ) {
				$this->Subroutinesbias = 107;
			} elseif ( $nSubrs < 33900 ) {
				$this->Subroutinesbias = 1131;
			} else {
				$this->Subroutinesbias = 32768;
			}
