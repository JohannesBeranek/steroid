...
class OTTTFont {
	...
	function set_filler_hash( $hash ) {
		$this->filler_hash = $hash;
	}
	...
	function __construct( $filename, $cache = true ) {
		...
		$this->fontfilelocation = $filename;
		$fh = $this->open();
		$this->version = fread( $fh, 4 );

		// TrueType collection?
		if ( $this->version == "ttcf" ) {
			trigger_error( "Tried to load a TrueType Collection as if it was a single font.\n", E_USER_ERROR );
		}
	...
	function is_TTF() {
		return ( $this->version == chr( 0 ) . chr( 1 ) . chr( 0 ) . chr( 0 ) );
	}

	// CFF fonts have the ascii string "OTTO" as version "number"
	function is_CFF() {
		return ( $this->version == "OTTO" );
	}
...
// ------------------- actual functions ------------------
	...
	function get_index( $character ) {
		// previously cached?
		if ( isset( $this->indexcache[ $character ] ) ) {
			return $this->indexcache[ $character ];
		}
