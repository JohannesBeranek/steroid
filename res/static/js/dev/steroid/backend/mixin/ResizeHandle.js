define(["../../../dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/connect", "dojo/_base/array", "dojo/_base/event",
	...
	"dojo/_base/declare"], function (kernel, lang, connect, arrayUtil, eventUtil, fxBase, windowBase, fxUtil, windowUtil, domUtil, domClass, domGeometry, domStyle, manager, Widget, TemplatedMixin, declare) {
	...
	var ResizeHandle = declare("dojox.layout.ResizeHandle", [Widget, TemplatedMixin],
		{
			...
			_updateSizing: function (/*Event*/ e) {
				...
				//TODO: make compatible with horizontal resizing once implemented

				if (me.activeResize) {
					me._changeSizing(e);
				} else {
					var tmp = me._getNewCoords(e, 'border');
					if (tmp === false) {
						return;
					}

					if (!me.pixelBased) {
						var theoreticalParentWidth = Math.max.apply(Math, me.owner.parentDimensions.w);
						var computedParentWidth = domGeometry.getMarginBox(me.owner.parentWidget[me.owner.parentWidget.resizeNode])['w'];

						var percent = tmp.w / theoreticalParentWidth;

						tmp.w = Math.floor(percent * computedParentWidth);
					}

					me._resizeHelper.resize(tmp);
				}
				e.preventDefault();
			},
			...
			_getNewCoords: function (/* Event */ e, /* String */ box) {
				...
				var dx = (this.isLeftToRight() ? 1 : -1) * (this.startPoint.x - e.clientX),
					...
					r = {
						w: this.owner.checkConstraints(newW, this.owner.parentDimensions.w),
						h: this.owner.checkConstraints(newH, this.owner.parentDimensions.h)
					}
					;
...
//		}
			...
			_changeSizing: function (/*Event*/ e) {
				...
				this.owner.setCurrentDimensions(tmp);
			},
