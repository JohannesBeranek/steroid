...
$mem=apc_sma_info();
if(empty($cache['num_hits'])) { $cache['num_hits']=1; $time++; }  // Avoid division by 0 errors on a cache clear
...
if (isset($MYREQUEST['IMG']))
{
	...
	case 2: 
		if (!isset($cache['num_misses'])) {  // PHP 5.5
			$cache['num_misses'] = 0;
		}
...
case OB_HOST_STATS:
	...
	if (!isset($cache['start_time'])) { // PHP 5.3
		$cache['start_time'] = 0;
	}

	if (!isset($cache['num_inserts'])) { // PHP 5.5
		$cache['num_inserts'] = 0;
	}

	if (!isset($cache_user['num_hits'])) { // PHP 5.5
		$cache_user['num_hits'] = 0;
	}

	if (!isset($cache_user['num_misses'])) { // PHP 5.5
		$cache_user['num_misses'] = 0;
	}

	if (!isset($cache_user['start_time'])) { // PHP 5.5
		$cache_user['start_time'] = 0;
	}

	if (!isset($cache_user['num_inserts'])) { // PHP 5.5
		$cache_user['num_inserts'] = 0;
	}

	if (!isset($cache['num_entries'])) { // PHP 5.5
		$cache['num_entries'] = 0;
	}

	if (!isset($cache_user['num_entries'])) { // PHP 5.5
		$cache_user['num_entries'] = 0;
	}

	if (!isset($cache['locking_type'])) { // PHP 5.5
		$cache['locking_type'] = 0;
	}

	if (!isset($cache['expunges'])) { // PHP 5.5
		$cache['expunges'] = 0;
	}

	if (!isset($cache_user['expunges'])) { // PHP 5.5
		$cache_user['expunges'] = 0;
	}
...
EOB;
  ...
  echo
	...
	foreach($cache[$scope_list[$MYREQUEST['SCOPE']]] as $i => $entry) {
		if (!isset($entry['num_hits'])) { // PHP 5.5
			$entry['num_hits'] = 0;
		}
		...
		if (!isset($entry['info'])) { // PHP 5.5
			$entry['info'] = NULL;	
		}

		if (!isset($entry['filename'])) { // PHP 5.5
			$entry['filename'] = NULL;
		}
	...
	if ($list) {
      ...
      if(!$MYREQUEST['SEARCH'] || preg_match($MYREQUEST['SEARCH'], $entry[$fieldname]) != 0) {  
		...
		if (!isset($entry['access_time'])) { // PHP 5.5
			$entry['access_time'] = NULL;
		}

		if (!isset($entry['creation_time'])) { // PHP 5.5
