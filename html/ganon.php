...
class HTML_Parser_Base extends Tokenizer_Base {
	...
	var $custom_char_map = array(
		'<' => self::TOK_TAG_OPEN,
		'>' => self::TOK_TAG_CLOSE,
// [Johannes Beranek 21.11.2013] should only be used inside tags, otherwise mismatched string delimiters will make problems
//		"'" => 'parse_string',
//		'"' => 'parse_string',
	...
	function parse_attributes() {
		// [Johannes Beranek 21.11.2013] add string delimiter when parsing attributes
		$this->custom_char_map["'"] = 'parse_string';
		$this->custom_char_map['"'] = 'parse_string';
		$this->buildCharMap();
			...
			if ($this->next_no_whitespace() === self::TOK_EQUALS) {
				if ($this->next_no_whitespace() === self::TOK_STRING) {
					...
					if (trim($val) === '') {
						...
						// [Johannes Beranek 21.11.2013] remove string delimiter again
						unset($this->custom_char_map["'"]);
						unset($this->custom_char_map['"']);
						$this->buildCharMap();
