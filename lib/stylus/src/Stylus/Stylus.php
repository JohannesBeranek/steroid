...
class Stylus
{
	...
	/**
	 * parseString - parses passed string and returns converted css
	 */
	...
	final private function prepare( $contents ) {
    ...
    public function parseFile($file, $overwrite = false)
    {
        ...
        if (preg_match('~(\.styl$)|(^[^\.]+$)~', $file)) {
            ...
            $contents = file_get_contents($filename) or StylusException::report('Could not read '.$filename);
 
			$parsed = $this->parseString( $contents );

            if ($parsed) {
				file_put_contents( $writename, $parsed ) or StylusException::report('Could not write to '.$writename);
            }
        }
